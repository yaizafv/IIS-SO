...
void Processor_InitializeInterruptVectorTable(int interruptVectorInitialAddress)
{
	...
	{ // Inicialice all to inicial IRET
	...
	interruptVectorTable[CLOCKINT_BIT] = interruptVectorInitialAddress + 2 * NUMBER_OF_CPU_INSTRUCTIONS_PER_INTERRUPT;
}
...
void Processor_DecodeAndExecuteInstruction()
{
	...
	case SHIFT_INST:
		...
		{ // SAL do not allow more than 31 bists shift...
			...
			if ((registerAccumulator_CPU & (1 << (sizeof(int) * 8 - 1))) ^ (oldAcc & (1 << (sizeof(int) * 8 - 1))))
			{ // some bit overflow...
	...
	case MEMADD_INST:
		...
		{
		case REGISTERA_CPU:
			segundoValor = registerA_CPU;
			break;
		case REGISTERB_CPU:
			segundoValor = registerB_CPU;
			break;
		case REGISTERACCUMULATOR_CPU:
		default:
			segundoValor = registerAccumulator_CPU;
			break;
		}
		Processor_CheckOverflow(registerMBR_CPU.cell, segundoValor, REGISTERACCUMULATOR_CPU);
		registerAccumulator_CPU = registerMBR_CPU.cell + segundoValor;
		registerPC_CPU++;
		break;
