...
int Heap_getValueUsedForOrdering(int, int);
...
// queueType: QUEUE_PRIORITY, QUEUE_ASSERT, ...
...
// Auxiliary function for secondaryKey comparations
...
void Heap_print(heapItem heap[], int queueType, int numElem) {
	int i,numberOfElements=numElem;
	heapItem *heapCopy = (heapItem *) malloc(numElem*sizeof(heapItem));
	memcpy(heapCopy, heap, numElem*sizeof(heapItem));

	for (i=0; i<numberOfElements-1; i++) {
		int PID=Heap_poll(heapCopy, queueType, &numElem);
		ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,40,SHORTTERMSCHEDULE,PID,Heap_getValueUsedForOrdering(PID, queueType));
	}
	if (i+1 == numberOfElements) {
		int PID=Heap_poll(heapCopy, queueType, &numElem);
		ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,41,SHORTTERMSCHEDULE,PID,Heap_getValueUsedForOrdering(PID, queueType));
	} else
		ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,100,SHORTTERMSCHEDULE,"\n");
}

int Heap_getValueUsedForOrdering(int id, int queueType) {
  int value=0;
  switch (queueType) {
	case QUEUE_PRIORITY:
		value= processTable[id].priority;
		break;
	case QUEUE_ASSERTS:
		value= asserts[id].time;
		break;
  }
  return value;
}
