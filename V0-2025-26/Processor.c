...
int _Helper_Read(int memAdress);
...
void Processor_DecodeAndExecuteInstruction()
{
	...
	// Instruction READ
	case READ_INST:
		registerAccumulator_CPU = _Helper_Read(operand1);
		registerPC_CPU++;
		break;
	...
	case MEMADD_INST:
		registerAccumulator_CPU = _Helper_Read(operand1) + operand2;
		registerPC_CPU++;
		break;
